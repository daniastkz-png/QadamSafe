                            ...
                            <div className="relative overflow-hidden rounded-2xl border-2 border-cyber-green/30 bg-background/50 backdrop-blur-sm shadow-2xl shadow-cyber-green/10">
                                ...
                                >
                                    {howItWorksSlides.map((slide, index) => (
                                        <div
                                            key={index}
                                            className={`w-full flex-shrink-0 flex items-center justify-center bg-cyber-green/5 ${(slide.step === 2 || slide.step === 3) ? 'p-4 md:p-6' : 'p-8 md:p-12'
                                                }`}
                                        >
                                            <img
                                                src={slide.image}
                                                alt={`Step ${slide.step}`}
                                                className={`h-auto object-contain rounded-xl shadow-2xl border border-white/10 ${(slide.step === 2 || slide.step === 3) ? 'w-full max-w-3xl' : 'w-full max-w-sm md:max-w-md'
                                                    }`}
                                            />
                                        </div>
                                    ))}
