# QadamSafe Backend API

Бэкенд API для QadamSafe, адаптированный для деплоя на Render.

## 🚀 Деплой на Render

### Шаг 1: Получите Firebase Service Account Key

1. Перейдите в [Firebase Console](https://console.firebase.google.com/project/qadamsafe/settings/serviceaccounts/adminsdk)
2. Нажмите **"Generate New Private Key"**
3. Скачайте JSON файл
4. **Сохраните содержимое** — оно понадобится для Render

### Шаг 2: Создайте репозиторий (если ещё нет)

Бэкенд должен быть в Git репозитории. Если проект уже в GitHub — отлично!

### Шаг 3: Создайте Web Service на Render

1. Перейдите на [render.com](https://render.com) и войдите/зарегистрируйтесь
2. Нажмите **"New +"** → **"Web Service"**
3. Подключите ваш GitHub/GitLab репозиторий
4. Настройте сервис:

| Настройка | Значение |
|-----------|----------|
| **Name** | `qadamsafe-api` |
| **Region** | Frankfurt (EU) — ближе к Казахстану |
| **Branch** | `main` |
| **Root Directory** | `backend` |
| **Runtime** | `Node` |
| **Build Command** | `npm install` |
| **Start Command** | `npm start` |
| **Plan** | Free |

### Шаг 4: Добавьте Environment Variables

В разделе **"Environment"** добавьте переменные:

| Key | Value |
|-----|-------|
| `FIREBASE_SERVICE_ACCOUNT` | Содержимое JSON файла из Шага 1 (в одну строку!) |
| `OPENAI_API_KEY` | Ключ с [platform.openai.com/api-keys](https://platform.openai.com/api-keys) (для gpt-4o-mini: сценарии, чат) |
| `JWT_SECRET` | `qadamsafe-secret-key-2024` |

> ⚠️ **Важно:** JSON нужно вставить как одну строку, без переносов!

### Шаг 5: Деплой

Нажмите **"Create Web Service"** и дождитесь деплоя (~2-3 минуты).

После деплоя вы получите URL вида:
```
https://qadamsafe-api.onrender.com
```

### Шаг 6: Обновите Frontend

После получения URL от Render, обновите `frontend/.env.production`:

```env
VITE_API_URL=https://qadamsafe-api.onrender.com
```

И пересоберите/задеплойте фронтенд.

---

## 🤖 Настройка Uptime Robot

Чтобы сервис не засыпал (бесплатный план Render усыпляет сервис после 15 минут неактивности):

1. Перейдите на [uptimerobot.com](https://uptimerobot.com)
2. Зарегистрируйтесь (бесплатно)
3. Нажмите **"Add New Monitor"**
4. Настройте:

| Настройка | Значение |
|-----------|----------|
| **Monitor Type** | HTTP(s) |
| **Friendly Name** | QadamSafe API |
| **URL** | `https://qadamsafe-api.onrender.com/health` |
| **Monitoring Interval** | 5 minutes |

5. Нажмите **"Create Monitor"**

✅ Теперь Uptime Robot будет пинговать ваш сервис каждые 5 минут, не давая ему заснуть!

---

## 🔧 Локальная разработка

1. Скопируйте `.env.example` в `.env`:
   ```bash
   cp .env.example .env
   ```

2. Заполните переменные в `.env`

3. Запустите сервер:
   ```bash
   npm run dev
   ```

4. API доступен на `http://localhost:3001`

---

## 📡 API Endpoints

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | `/health` | Health check для Uptime Robot |
| POST | `/api/auth/register` | Регистрация |
| POST | `/api/auth/login` | Вход |
| GET | `/api/auth/me` | Текущий пользователь |
| GET | `/api/scenarios` | Список сценариев |
| POST | `/api/ai/generate-scenario` | Генерация AI сценария |
| GET | `/api/ai/topics` | Темы для AI сценариев |

---

## 🛡️ Безопасность

- JWT токены действуют 7 дней
- Пароли хешируются bcrypt
- CORS настроен для вашего домена
